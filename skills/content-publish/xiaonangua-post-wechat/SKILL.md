---
name: xiaonangua-post-wechat
description: Xiaonangua fork of baoyu-post-to-wechat. Posts content to WeChat Official Account (微信公众号) via API or Chrome CDP. Supports article posting (文章) with HTML/Markdown and image-text posting (图文).
---

# Xiaonangua Post to WeChat Official Account
...
This skill is a **verbatim copy** of `baoyu-post-to-wechat` at the time of cloning, with only naming/metadata adjusted.
...
test -f .baoyu-skills/xiaonangua-post-wechat/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.baoyu-skills/xiaonangua-post-wechat/EXTEND.md" && echo "user"
```

┌────────────────────────────────────────────────────────┬───────────────────┐
│                          Path                          │     Location      │
├────────────────────────────────────────────────────────┼───────────────────┤
│ .baoyu-skills/xiaonangua-post-wechat/EXTEND.md           │ Project directory │
├────────────────────────────────────────────────────────┼───────────────────┤
│ $HOME/.baoyu-skills/xiaonangua-post-wechat/EXTEND.md     │ User home         │
└────────────────────────────────────────────────────────┴───────────────────┘
...
- **Image placeholders that survive WeChat paste**
  - When loading HTML, the script injects a visible text marker `MDTOHTMLIMGPH_N` **before** each placeholder `<img src="MDTOHTMLIMGPH_N">`.
  - After paste, the script finds the marker text → deletes it → pastes the real local image.

- **Mac terminal dark code blocks (no global CSS)**
  - WeChat often strips `<style>`; the script forces **inline styles** for code blocks:
    - `pre.hljs.code__pre` → `background:#0b1020; color:#E5E7EB`.

- **Image insertion is verifiable + retry**
  - After each image paste, the script waits for the editor `<img>` count to increase; if not, it retries paste (up to 3 times).

- **Body paste fallback (avoid “textLength=8” false success)**
  - If Cmd+V paste results in a suspiciously short body, the script falls back to `document.execCommand('insertHTML', ...)`.

- **Avoid `npx bun` ENOTEMPTY**
  - For markdown conversion in this skill, prefer calling `bun` directly when possible.
